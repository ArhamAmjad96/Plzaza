...
import { getLocalBillFile, generateBillFilename } from "@/lib/bills/bill-storage";
...
export async function GET(
  ...
  try {
    ...
    if (referenceNumber) {
      ...
      // Persist for future instant downloads
      if (bill) {
        await saveBillFile({
          buffer: pngBuffer,
    ...
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to download bill." },
      { status: 500 }
    );
  }
}

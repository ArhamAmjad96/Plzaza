...
const html = fs.readFileSync("test-bill-result.html", "utf8");
...
function getLatePaymentValues() {
  ...
  result.lp_surcharge_period = clean(
    firstColumn
  ...
  result.payable_after_due_date = numberOrNull(
    firstColumn
  ...
  if (secondColumn.length) {
    ...
    result.second_lp_surcharge = numberOrNull(
      secondColumn
        .find(".lp-surcharge-top-val")
        .first()
        .text()
    ...
    result.payable_after_lp_period = clean(
      secondColumn
        .find(".lp-surcharge-period")
        .first()
        .text()
    ...
    result.payable_after_lp_date = numberOrNull(
      secondColumn
        .find(".lp-surcharge-bottom-val")
        .first()
        .text()
...
const bill = {
  ...
  net_electricity_charges:
    numberOrNull(
      getChargeValue("NET ELECTRICITY CHARGES")
    ),
